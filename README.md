...
2. OSV.dev に問い合わせ、CISA KEV カタログと照合する
3. `config.yml` の `ignore` を適用する（期限切れの除外は自動で失効させる）
4. **各 CVE についてコードを読み、脆弱な機能を実際に使っているか判定する** ← 中核
5. 要対応 / 様子見 / 影響なし に分類する
6. `reports/YYYY-MM-DD.md` を書く
7. `logs/triage.jsonl` に 1 判定 1 行で追記する
8. 条件を満たせば GitHub Issue を起票する（既定では無効）
9. 保持期間を過ぎた古いレポートを月次サマリーに集約して削除する
